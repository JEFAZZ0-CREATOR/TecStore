...
import {
  FiEdit2, FiSave, FiX, FiLogOut, FiKey, FiUser, FiShield, FiSettings,
  FiCamera, FiMail, FiPhone, FiMapPin, FiEye, FiEyeOff, FiAlertCircle,
  FiCheckCircle, FiShoppingBag, FiDollarSign, FiStar, FiCalendar,
  FiRefreshCw
} from 'react-icons/fi'
...
const getAvatarSrc = (url, name) => {
  if (!url) return `https://ui-avatars.com/api/?name=${encodeURIComponent(name ?? 'U')}&background=6d28d9&color=fff&size=128`
...
function StatCard({ icon: Icon, label, value, color, delay }) {
  return (
    <motion.div
      ...
      style={{ background: 'linear-gradient(135deg,rgba(20,20,60,.85),rgba(12,12,32,.9))' }}
    >
...
function PasswordInput({ value, onChange, placeholder, name, error }) {
  ...
  return (
    <div className="space-y-1">
      <div className="relative">
        <FiKey size={14} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-500" />
        <input
...
export default function Profile() {
  ...
  const avatarSrc = avatarPreview
    ?? getAvatarSrc(user?.avatarUrl, user?.name)
  ...
  const handlePassChange = async (e) => {
    ...
    if (passForm.newPass !== passForm.confirm) errs.confirm = 'Las contraseñas no coinciden'
    setPassErrors(errs)
    if (Object.keys(errs).length) return

    setPassLoad(true)
    try {
      const updated = await usersAPI.updateProfile({ password: passForm.newPass })
      setUser(updated)
  ...
  const totalOrders = stats?.totalSessions ?? stats?.count ?? stats?.totalPurchases ?? 0
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      ...
      <motion.div
        ...
        style={{ background: 'linear-gradient(145deg,rgba(20,20,60,.9),rgba(10,10,28,.95))' }}
      >
        {/* Top bar */}
        <div className="h-1 w-full" style={{ background: 'linear-gradient(90deg,#6d28d9,#3b82f6,#22d3ee)' }} />
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <StatCard icon={FiShoppingBag} label="Compras registradas"
          value={statsLoading ? '—' : totalOrders}
          color="#3b82f6" delay={0.05} />
        <StatCard icon={FiDollarSign} label="Total invertido"
          value={statsLoading ? '—' : fmtMoney(totalSpent)}
          color="#22d3ee" delay={0.1} />
        <StatCard icon={FiStar} label="Puntos de fidelidad"
          value={statsLoading ? '—' : Math.round(totalSpent / 10)}
          color="#f59e0b" delay={0.15} />
        <StatCard icon={FiCalendar} label="Último ingreso"
          value={user?.lastLoginAt ? new Date(user.lastLoginAt).toLocaleDateString('es-MX', { day:'2-digit', month:'short' }) : '—'}
          color="#10b981" delay={0.2} />
      </div>

      {/* ─── Tabs ──────────────────────────────────────────────────────────── */}
      <div className="rounded-3xl border border-white/10 overflow-hidden"
        style={{ background: 'linear-gradient(145deg,rgba(18,18,50,.92),rgba(10,10,28,.96))' }}>
        ...
        <AnimatePresence mode="wait">
          ...
          >
            ...
            {activeTab === 'perfil' && (
              <div className="space-y-5">
                <div className="flex items-center justify-between">
                  <h2 className="text-base font-bold text-white">Información personal</h2>
                  <button
                    onClick={() => { setIsEditing(v => !v); if (isEditing && user) setFormData({ name: user.name||'', email: user.email||'', phone: user.phone||'', address: user.address||'' }) }}
                  ...
                  >
                    {isEditing ? <><FiX size={12} /> Cancelar</> : <><FiEdit2 size={12} /> Editar</>}
                  </button>
                </div>

                <div className="grid sm:grid-cols-2 gap-4">
                  <Field label="Nombre completo" icon={FiUser}>
                    <TextInput icon={FiUser} name="name" disabled={!isEditing}
                      value={formData.name} onChange={e => setFormData(p => ({ ...p, name: e.target.value }))}
                      placeholder="Tu nombre" />
                  </Field>
                  <Field label="Correo electrónico" icon={FiMail}>
                    <TextInput icon={FiMail} name="email" type="email" disabled={!isEditing}
                      value={formData.email} onChange={e => setFormData(p => ({ ...p, email: e.target.value }))}
                      placeholder="tu@correo.com" />
                  </Field>
                  <Field label="Teléfono" icon={FiPhone}>
                    <TextInput icon={FiPhone} name="phone" disabled={!isEditing}
                      value={formData.phone} onChange={e => setFormData(p => ({ ...p, phone: e.target.value }))}
                      placeholder="Tu teléfono" />
                  </Field>
                  <Field label="Dirección" icon={FiMapPin}>
                    <TextInput icon={FiMapPin} name="address" disabled={!isEditing}
            ...
            {activeTab === 'seguridad' && (
              <div className="space-y-5">
                <div>
                  <h2 className="text-base font-bold text-white mb-0.5">Cambiar contraseña</h2>
                  <p className="text-slate-500 text-xs">Elige una contraseña segura de al menos 6 caracteres.</p>
                </div>

                <form onSubmit={handlePassChange} className="space-y-4 max-w-sm">
                  <Field label="Contraseña actual">
                    <PasswordInput
                      name="current" placeholder="Tu contraseña actual"
                  ...
                  <Field label="Nueva contraseña">
                    <PasswordInput
                      name="newPass" placeholder="Nueva contraseña"
                  ...
                  <Field label="Confirmar nueva contraseña">
                    <PasswordInput
                      name="confirm" placeholder="Confirmar contraseña"
                ...
                <div className="mt-6 pt-5 border-t border-white/8 space-y-3">
                  <h3 className="text-sm font-semibold text-white">Estado de la cuenta</h3>
            ...
            {activeTab === 'preferencias' && (
              <div className="space-y-5">
                <div>
                  <h2 className="text-base font-bold text-white mb-0.5">Notificaciones</h2>
                  <p className="text-slate-500 text-xs">Controla qué comunicaciones recibes de TecStore.</p>
                </div>

                <div className="rounded-2xl border border-white/8 px-4 divide-y divide-white/5"
                  ...
                  <PrefToggle
                    ...
                    onChange={v => setPrefs(p => ({ ...p, offerAlerts: v }))}
                  />
                  <PrefToggle
                    ...
                    onChange={v => setPrefs(p => ({ ...p, priceDropAlerts: v }))}
                  />
                  <PrefToggle
                ...
                <div className="pt-2">
                  <h2 className="text-base font-bold text-white mb-3">Cuenta</h2>
                  <div className="space-y-2">
                    <button className="w-full flex items-center gap-3 px-4 py-3 rounded-xl border border-white/8
                    ...
                    <button className="w-full flex items-center gap-3 px-4 py-3 rounded-xl border border-danger/15
