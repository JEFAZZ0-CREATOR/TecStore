...
const PLACEHOLDER_IMG =
  "data:image/svg+xml,%3Csvg xmlns=%22http://www.w3.org/2000/svg%22 width=%22200%22 height=%22200%22 viewBox=%220 0 200 200%22%3E%3Crect width=%22200%22 height=%22200%22 fill=%22%23263a52%22/%3E%3Ctext x=%2250%25%22 y=%2250%25%22 dominant-baseline=%22middle%22 text-anchor=%22middle%22 fill=%22%23ffffff%22 font-family=%22Arial%22 font-size=%2216%22%3EImagen%20no%20disponible%3C/text%3E%3C/svg%3E";
...
export const ProductCard = ({
  ...
  const externalUrl =
    product.url && product.url.startsWith("http") ? product.url : null;
  ...
  const ContentWrapper = ({ children }) =>
    externalUrl ? (
      ...
      <Link
        ...
        className="relative w-full h-48 bg-slate-800 overflow-hidden block"
      >
        {children}
      </Link>
    );
    ...
    >
      <GlassCard className="overflow-hidden h-full flex flex-col">
        <ContentWrapper>
          <img
            ...
            onError={(e) => {
              ...
              e.target.src =
                "https://placehold.co/400x300/1a1a3e/6d28d9?text=" +
                encodeURIComponent(product.title?.slice(0, 15) || "Producto");
            }}
            className="w-full h-full object-cover"
          />
        ...
        <div className="flex-1 p-3 flex flex-col justify-between">
          <div>
            <p className="text-xs text-accent mb-1 capitalize">
    ...
    return (
      ...
      >
        {[...Array(viewMode === "list" ? 6 : 8)].map((_, i) => (
          <div
            key={i}
            className={`card animate-pulse ${viewMode === "list" ? "h-28" : "h-80"}`}
          />
        ))}
      </div>
    );
